...
public class FSMetricsCollector implements MetricsCollector {
  ...
  public HashMap<String, Long> collect(FileRepository repository, Project.NameKey projectNameKey) {
    ...
    HashMap<String, AtomicInteger> partialMetrics =
        filesAndDirectoriesCount(repository, projectNameKey, metrics);
    putMetric(
        projectNameKey,
        metrics,
    ...
    putMetric(
        projectNameKey, metrics, numberOfFiles, partialMetrics.get(numberOfFiles).longValue());
    putMetric(
        projectNameKey,
        metrics,
        numberOfKeepFiles,
        partialMetrics.get(numberOfKeepFiles).longValue());

    return metrics;
  }

  private HashMap<String, AtomicInteger> filesAndDirectoriesCount(
      FileRepository repository, Project.NameKey projectNameKey, HashMap<String, Long> metrics) {
    ...
    try {
      ...
      logger.atSevere().withCause(e).log(
          "Can't open object directory for project " + projectNameKey.get());
    }
