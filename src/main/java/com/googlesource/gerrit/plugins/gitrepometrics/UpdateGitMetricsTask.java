  ...
  public void run() {
    ...
    try (Repository repository = repoManager.openRepository(projectNameKey)) {
      ...
      StreamSupport.stream(iterable.spliterator(), false)
          .forEach(
              metricsCollector -> {
                HashMap< GitRepoMetric, Long> metrics = metricsCollector.collect((FileRepository) unwrappedRepo, project);
                metrics.forEach((k, v) -> {
                  logger.atSevere().log(String.format("Collected %s for project %s: %d", k.getName(), projectName, v));
                  gitRepoMetricsCache.setMetric(k,v, projectName);
                });
              }
          );
