    ...
    executorService.submit(
        () -> {
          ...
          try {
            ...
            GitRepositoryStatistics.RepositoryStatistics repoStatistics =
                GitRepositoryStatistics.collectFor(repository);
            metrics.put(numberOfPackFilesAfterBitmap, repoStatistics.numberOfPackFilesAfterBitmap);
            logger.atInfo().log(
                "New Git Statistics metrics collected: %s, %s",
                statistics.toString(), repoStatistics);
          } catch (IOException e) {
