...
public class RepoCountMetricRegister implements LifecycleListener {
  ...
  private RegistrationHandle rh = null;
  ...
  public void start() {  
    if (metricExists(REPO_COUNT_METRIC_NAME)) {
      logger.atInfo().log("Removing metric " + REPO_COUNT_METRIC_NAME);
      metricRegistry.remove(
          String.format("%s/%s/%s", "plugins", "git-repo-metrics", REPO_COUNT_METRIC_NAME));
    }

    logger.atInfo().log("Registering metric " + REPO_COUNT_METRIC_NAME);

    rh = metricMaker.newCallbackMetric(
  ...
  @Override
  public void stop() {
	  synchronized (metricMaker) {
		  if (rh != null) {
			  logger.atInfo().log("Removing handle for metric " + REPO_COUNT_METRIC_NAME);
			  rh.remove();			  
		  }
	  }
  }
