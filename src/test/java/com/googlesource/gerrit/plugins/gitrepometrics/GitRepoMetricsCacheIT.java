...
public class GitRepoMetricsCacheIT extends LightweightPluginDaemonTest {
  ...
  @GlobalPluginConfig(
      ...
      name = "git-repo-metrics.project",
      value = "testProject1")
  @GerritConfig(name = "gerrit.instanceId", value = "test-instance-id")
  ...
  @GlobalPluginConfig(
      ...
      value = "1 s")
  @GerritConfig(name = "gerrit.instanceId", value = "test-instance-id")
  public void shouldUpdateRepositoryMetricsOnTwoRefUpdatesAndGracePeriod() throws Exception {
    addTwoBranchesToProjectAndAssertLooseRefsMetrics();
  }

  private void addTwoBranchesToProjectAndAssertLooseRefsMetrics()
      throws InterruptedException, RestApiException {
    long initialLooseRefs = waitForMetricValue(testProject1.get(), NUM_LOOSE_REFS, (v) -> true);
  ...
  private Optional<Long> gitRepoMetric(String projectName, String metricName) {
    return gitRepoMetricsCache.getMetrics().entrySet().stream()
        .filter(
            metricEntry ->
                metricEntry
                    .getKey()
                    .toLowerCase()
                    .equals(metricName + "_" + projectName.toLowerCase()))
