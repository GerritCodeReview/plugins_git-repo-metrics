...
import static com.googlesource.gerrit.plugins.gitrepometrics.collectors.FSStats.numberOfDirectories;
import static com.googlesource.gerrit.plugins.gitrepometrics.collectors.FSStats.numberOfEmptyDirectories;
import static com.googlesource.gerrit.plugins.gitrepometrics.collectors.FSStats.numberOfFiles;
import static com.googlesource.gerrit.plugins.gitrepometrics.collectors.FSStats.numberOfKeepFiles;
...
public class FSStatsTest {
  ...
  public void testCorrectMetricsCollection() throws IOException {
    ...
    HashMap<GitRepoMetric, Long> metrics = new FSStats().collect((FileRepository) repository, project);
    ...
    //  └── pack
    //      └── keep1.keep
    assertThat(metrics.get(numberOfKeepFiles)).isEqualTo(1); // keep1.keep
    assertThat(metrics.get(numberOfFiles)).isEqualTo(1); // keep1.keep
    assertThat(metrics.get(numberOfDirectories)).isEqualTo(3); // info, pack and .
    assertThat(metrics.get(numberOfEmptyDirectories)).isEqualTo(1); // info
  }
