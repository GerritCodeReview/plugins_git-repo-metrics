...
public class GitUpdateListenerTest {
  ...
  public void shouldTriggerRateLimiter() {
    TestProjectMetricsThrottler testProjectMetricsThrottler = new TestProjectMetricsThrottler();
    GitRepoUpdateListener limitedGitRepoUpdateListener = new GitRepoUpdateListener(
        producerInstanceId,
        mockedExecutorService,
        updateGitMetricsTaskFactory,
        gitRepoMetricsCache,
        testProjectMetricsThrottler
        );
    ...
    try {
      WaitUtil.waitUntil(
          () ->
              testProjectMetricsThrottler.acquireCount.get() == 1,
          Duration.ofSeconds(MAX_WAIT_TIME_FOR_METRICS_SECS));
    } catch (InterruptedException e) {
      fail(
          String.format(
              "Rate limiter not triggered for project %s", enabledProject));
    }
  }

  public static class TestProjectMetricsThrottler implements ProjectMetricsLimiter {
